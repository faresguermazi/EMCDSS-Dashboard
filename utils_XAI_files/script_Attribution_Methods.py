...
class EHRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(si, torch.Tensor):
            si = torch.tensor(si, dtype=torch.float)
        else:
            si = si.float()
...
from transformers import AutoTokenizer
from captum.attr import IntegratedGradients, LayerIntegratedGradients, DeepLiftShap, LayerDeepLift
from captum.attr._utils.input_layer_wrapper import ModelInputWrapper
...
class InputWrapper(ModelInputWrapper):
    def __init__(self, model):
...
# ==========================
torch.backends.cudnn.enabled=False
SAVE_DIR = "/workspace/XAI/utils_generated_files"
...
experiment_path = '/workspace/XAI/configs/experiment'
...
for l, c in zip(loops, configs):
    ...
    check_point = config['ckpt_path']

    model_class = globals().get(model_name)

    # Data module
    dm = MIMICDataModule(notes=(dataset == 'notes'), discharge=(dataset == 'discharge'),
                         time_series=True, batch_size=1)
    dm.prepare_data()
    dm.setup()
    ...
    for method_name, explainer_class in METHODS.items():
        print(f"\n=== Running {method_name} ===")

        # Reload model fresh for each method
        ...
        # Wrap model
        if method_name in ["IG", "SHAP"]:
            wrapped_model = EmbeddingInputWrapper(model)
        else:  # LIG, LayerDeepLift
            wrapped_model = InputWrapper(model)

        # Initialize explainer
        if method_name in ["LIG", "LayerDeepLift"]:
            target_layer = wrapped_model.module.nt_enc.bert.embeddings
            explainer = explainer_class(wrapped_model, target_layer)
        else:
            explainer = explainer_class(wrapped_model)

        # Output file
        output_path = os.path.join(SAVE_DIR, f"{method_name}_attribution_output.jsonl")
        if os.path.exists(output_path):
            os.remove(output_path)
        file_out = open(output_path, "w+")

        # Attribution loop
        for input_ids, si, xi, token_type_ids, attention_mask, _, _ in tqdm(dm.test_dataloader(), desc=method_name):
            ...
            base_time_series = ((0 * xi) + -1).to(device)
            base_time_invarient = ((0 * si) + -1).to(device)
            ...
            if method_name in ["IG", "SHAP"]:
                ...
                if method_name == "SHAP":
                    nbr_samples = 8
                    base_embeds = base_embed.repeat(nbr_samples, 1, 1)
                    base_time_series_rep = base_time_series.repeat(nbr_samples, 1, 1)
                    base_time_invarient_rep = base_time_invarient.repeat(nbr_samples, 1)
                    baselines = (base_embeds, base_time_invarient_rep, base_time_series_rep)
                else:
                    baselines = (base_embed, base_time_invarient, base_time_series)
                inputs = (embeddings, si, xi)
            else:
                baselines = (base_reference_text, base_time_invarient, base_time_series)
                inputs = (input_ids, si, xi)

            add_args = (token_type_ids, attention_mask)

            try:
                if method_name in ["LIG", "IG"]:
            ...
            if not isinstance(attribution, (tuple, list)):
                attribution = (attribution,)

            # Unpack correctly
            attribution_s = attribution[0].squeeze() if len(attribution) > 0 else None
            attribution_X = attribution[1].squeeze() if len(attribution) > 1 else None
            attribution_txt = attribution[2].squeeze() if len(attribution) > 2 else None

            relevancies = {
                'attri_s': attribution_s.cpu().detach().numpy().tolist() if attribution_s is not None else [],
                'attri_X': attribution_X.cpu().detach().numpy().tolist() if attribution_X is not None else [],
                'attri_txt_norm': torch.norm(attribution_txt, dim=-1).cpu().detach().numpy().tolist() if attribution_txt is not None else [],
                'attri_txt_mean': torch.mean(attribution_txt, dim=-1).cpu().detach().numpy().tolist() if attribution_txt is not None else [],
            }

            file_out.write(json.dumps(relevancies) + '\n')

        file_out.close()
        print(f"✅ Saved {method_name} attributions to {output_path}")
