...
for l, c  in zip(loops,configs):
    ...
    with torch.no_grad():
        for input_ids, si, xi, token_type_ids, attention_mask, y, icu_id in tqdm(test_loader, desc="Collecting embeddings"):
            ...
            test_embeddings.append(emb_np)
            test_labels.append(y_np)
            test_icu_ids.append(icu_np)

    # Concatenate
    test_embeddings = np.concatenate(test_embeddings, axis=0)  # shape [N, embed_dim]
    test_labels     = np.concatenate(test_labels, axis=0)      # shape [N]
    test_icu_ids    = np.concatenate(test_icu_ids, axis=0)     # shape [N]

    print("Done collecting. Embeddings shape:", test_embeddings.shape)

    data_list = []
    for i in range(len(test_embeddings)):
        data_list.append({
            "icu": int(test_icu_ids[i]),
            "label": float(test_labels[i]),
            "embedding": test_embeddings[i].tolist()  # convert from np array -> Python list
        })

    with open("test_data.json", "w") as f: #CHANGE PATH IF NEEDED
