...
class MBertLstm(nn.Module):
    ...
    def get_fused_embedding(self, input_ids, s, X, token_type_ids, attention_mask):
        """
        Returns the penultimate layer embedding
        (the fused representation) BEFORE the final
        classification layer (self.pred).
        """
        s_enc = self.s_enc(s)
        X_enc = self.X_enc(X)
        ...
        fusion = self.gate(nt, s_enc, X_enc)  # shape: [batch_size, bert_size]
        return fusion
...
for l, c  in zip(loops,configs):
    ...
    with torch.no_grad():
        for input_ids, si, xi, token_type_ids, attention_mask, y, icu_id in tqdm(train_loader, desc="Collecting embeddings"):
            ...
            emb = model.get_fused_embedding(input_ids, si, xi, token_type_ids, attention_mask)
            emb_np = emb.cpu().numpy()       # shape: [batch_size, embed_dim]
            y_np   = y.numpy()               # shape: [batch_size]
            icu_np = icu_id.numpy()          # shape: [batch_size]

            train_embeddings.append(emb_np)
            train_labels.append(y_np)
            train_icu_ids.append(icu_np)

    # Concatenate
    train_embeddings = np.concatenate(train_embeddings, axis=0)  # shape [N, embed_dim]
    train_labels     = np.concatenate(train_labels, axis=0)      # shape [N]
    train_icu_ids    = np.concatenate(train_icu_ids, axis=0)     # shape [N]

    print("Done collecting. Embeddings shape:", train_embeddings.shape)

    data_list = []
    for i in range(len(train_embeddings)):
        data_list.append({
            "icu": int(train_icu_ids[i]),
            "label": float(train_labels[i]),
            "embedding": train_embeddings[i].tolist()  # convert from np array -> Python list
        })

    with open("train_data.json", "w") as f: #CHANGE PATH IF NEEDED
        json.dump(data_list, f)  # no indent to keep it smaller
