    ...
    def __getitem__(self, index):
       ...
       # si = torch.from_numpy(si).float()
        ...
        if (not self.notes and not self.discharge) and self.time_series:
            ...
            if self.time_series:
                si = torch.from_numpy(si).float()
                print("DEBUG xi type:", type(xi), xi.shape)  # or use logging
                assert isinstance(xi, torch.Tensor), "xi is not a Tensor!"
                #xi = torch.from_numpy(xi).float()
...
for l, c  in zip(loops,configs):
    ...
    wrapped_model = ModelInputWrapper(model)
    
    # (2) Identify layers:
    layer_s   = wrapped_model.module.s_enc
    layer_x   = wrapped_model.module.X_enc.lstm
    layer_txt = wrapped_model.module.nt_enc.bert.embeddings

    # (3) Create three LayerDeepLift objects:
    explainer_s   = LayerDeepLift(wrapped_model, layer_s)
    explainer_x   = LayerDeepLift(wrapped_model, layer_x)
    explainer_txt = LayerDeepLift(wrapped_model, layer_txt)

    # output file to wite the attributions
    file_out = open('DeepLift_attribution_output_' + model_name + '.jsonl', 'w+') #CHANGE PATH IF NEEDED

    counter = 0
    # run on whole test data, it is better to run it for one data point to check for any issues
    for input_ids, si, xi, token_type_ids, attention_mask, _, _ in tqdm(dm.test_dataloader()):

        # initialize relevancies
        relevancies = {}

        # get the index of <SEP> token
        ...
        base_reference_text = torch.tensor(base_reference_text).unsqueeze(0).to(device)

        base_time_series = ((0 * xi) + -1).to(device)
        base_time_invarient = ((0 * si) + -1).to(device)

        base_reference = (base_reference_text, base_time_invarient, base_time_series)
        ...
        attr_s = explainer_s.attribute(
        inputs=(input_ids, si, xi),
        baselines=base_reference,
        additional_forward_args=add_args
        )
        
        attr_x = explainer_x.attribute(
            inputs=(input_ids, si, xi),
            baselines=base_reference,
            additional_forward_args=add_args,
        ...
        attr_txt = explainer_txt.attribute(
            inputs=(input_ids, si, xi),
            baselines=base_reference,
            additional_forward_args=add_args
        )
        ...
        # static data
        attr_s = attr_s.squeeze().detach().cpu().numpy().tolist()
        relevancies["attri_s"] = attr_s

        # time-series
        attr_x = attr_x.squeeze().detach().cpu()
        relevancies["attri_X"] = attr_x.numpy().tolist()

        # text embeddings
        attr_txt = attr_txt.squeeze().detach().cpu()
        attr_txt_norm = torch.norm(attr_txt, dim=-1).numpy().tolist()
        attr_txt_mean = torch.mean(attr_txt, dim=-1).numpy().tolist()

        relevancies["attri_txt_norm"] = attr_txt_norm
        relevancies["attri_txt_mean"] = attr_txt_mean
