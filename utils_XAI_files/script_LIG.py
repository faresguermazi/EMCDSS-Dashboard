...
for l, c  in zip(loops,configs):
    ...
    explainer = LayerIntegratedGradients(wrapped_model, [wrapped_model.input_maps["s"], wrapped_model.input_maps["X"], wrapped_model.module.nt_enc.bert.embeddings])

    # output file to wite the attributions
    file_out = open('LIG_attribution_output' + model_name + '.jsonl', 'w+') #CHANGE PATH IF NEEDED
    ...
    for input_ids, si, xi, token_type_ids, attention_mask, _, _ in tqdm(dm.test_dataloader()):
        ...
        # get the attribution
        attribution = explainer.attribute(inputs=(input_ids,si,xi),
        ...
        # LIG relevances
        attribution_s = attribution[0].squeeze()
        attribution_X = attribution[1].squeeze()
        attribution_txt = attribution[2].squeeze()
